...
public class AdminController {
    ...
    public AdminController(UsersRepository usersRepository, PasswordEncoder passwordEncoder) {
    ...
    public UserResponse addUserByAdmin(@Valid @RequestBody UserCreateRequest request) {
        ...
        if (usersRepository.findByUsername(request.getUsername()).isPresent()) {
            throw new ConflictException("USERNAME_TAKEN", "Le nom d'utilisateur « " + request.getUsername() + " » est déjà utilisé.");
        }
    ...
    public ResponseEntity<UserResponse> getUserById(@PathVariable Integer id) {
        ...
        Users user = usersRepository.findById(id)
            .orElseThrow(() -> new NotFoundException("USER_NOT_FOUND", "User with id " + id + " does not exist."));
    ...
    public ResponseEntity<UserResponse> updateUser(@PathVariable Integer id, @Valid @RequestBody UserCreateRequest userDetails) {
        ...
        if (usernamePrisParUnAutre) {
            throw new ConflictException("USERNAME_TAKEN", "Le nom d'utilisateur « " + userDetails.getUsername() + " » est déjà utilisé.");
        }
