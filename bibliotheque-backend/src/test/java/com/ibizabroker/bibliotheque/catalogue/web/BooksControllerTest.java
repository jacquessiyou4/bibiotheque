...
import com.ibizabroker.bibliotheque.catalogue.internal.BooksService;
...
class BooksControllerTest {
    ...
    void createBook_creeLeLivreEtRenvoieLeDto() {
        BookRequest demande = demande("Nouveau", 4);
    ...
    void createBook_sansAuthentification_journaliseAnonymeSansErreur() {
        BookRequest demande = demande("Nouveau", 1);
    ...
    void updateBook_renvoieLeLivreModifie() {
        BookRequest modification = demande("L1 bis", 7);
        when(booksService.update(1, modification)).thenReturn(livre(1, "L1 bis", 7));
    ...
    private BookRequest demande(String nom, int exemplaires) {
        return new BookRequest(nom, "Auteur", "Roman", exemplaires);
    }
