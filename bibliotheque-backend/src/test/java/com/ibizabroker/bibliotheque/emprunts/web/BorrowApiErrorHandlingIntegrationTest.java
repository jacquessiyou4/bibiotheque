...
class BorrowApiErrorHandlingIntegrationTest {
    ...
    void setUp() {
        ...
        when(booksRepository.decrementerStock(3)).thenReturn(1);
    ...
    void postBorrow_stockEpuise_renvoie400EtNeSauvegardePasLaCopie() throws Exception {
        ...
        when(booksRepository.decrementerStock(3)).thenReturn(0);
    ...
    @Test
    void putBorrow_sansBorrowId_renvoie400DeValidation() throws Exception {
        mockMvc.perform(put("/borrow")
                        .header(HttpHeaders.AUTHORIZATION, BEARER_ADHERENT)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{}"))
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.errors.borrowId").exists());
    }
