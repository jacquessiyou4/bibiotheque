...
class UsersAdminApiIntegrationTest {
    ...
    void avecTokenAdmin_postUserAvecUsernameDejaUtilise_renvoie409EtNeSauvegardeRien() throws Exception {
        // Le username est comparé sous sa forme canonique : « A1 » saisi = « a1 » en base.
    ...
    @Test
    void avecTokenAdmin_postUser_enregistreLaFormeCanoniqueDuUsernameEtDuNom() throws Exception {
        mockMvc.perform(post("/admin/users")
                        .header(HttpHeaders.AUTHORIZATION, "Bearer " + TOKEN_ADMIN)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{\"username\":\"  Marie \",\"name\":\" Marie   Curie \",\"password\":\"secret1\"}"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.username").value("marie"));

        ArgumentCaptor<Users> sauvegarde = ArgumentCaptor.forClass(Users.class);
        verify(usersRepository).save(sauvegarde.capture());
        assertThat(sauvegarde.getValue().getUsername()).isEqualTo("marie");
        assertThat(sauvegarde.getValue().getName()).isEqualTo("Marie Curie");
    }

    @Test
    void erreurDeValidation_renvoieUnCorpsProblemJson() throws Exception {
        mockMvc.perform(post("/admin/users")
                        .header(HttpHeaders.AUTHORIZATION, "Bearer " + TOKEN_ADMIN)
                        .header("X-Request-ID", "req-test-42")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{\"name\":\"Sans Username\"}"))
                .andExpect(status().isBadRequest())
                .andExpect(content().contentTypeCompatibleWith("application/problem+json"))
                .andExpect(jsonPath("$.status").value(400))
                .andExpect(jsonPath("$.code").value("VALIDATION_FAILED"))
                .andExpect(jsonPath("$.requestId").value("req-test-42"))
                .andExpect(jsonPath("$.errors.username").exists());
    }
