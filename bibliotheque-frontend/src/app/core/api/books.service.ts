...
import { DEFAULT_PAGE_SIZE, LIST_PAGE_SIZE, Page } from '../../shared/models/page';
import { apiUrl } from '../services/api-config';

// Corps attendu par POST et PUT /admin/books : l'identifiant vient de l'URL.
export function toBookRequest(book: Books): Omit<Books, 'bookId'> {
  return {
...
export class BooksService {
  ...
  private baseURL = `${apiUrl()}/api/v1/books`;
  ...
  createBook(book: Books): Observable<Object> {
    return this.httpClient.post(`${this.baseURL}`, toBookRequest(book));
  }
  ...
  updateBook(bookId: number, book: Books): Observable<Object> {
    return this.httpClient.put(`${this.baseURL}/${bookId}`, toBookRequest(book));
  }
